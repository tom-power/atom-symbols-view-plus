...
export default class SymbolsView {
  ...
  elementForItem({position, name, kind, file, directory}) {
    ...
    if (this.theme === 'modal') {
      ...
      const path = file.replace(directory, '')
      secondaryLine.classList.add('secondary-line');
      if (position) {
        secondaryLine.textContent = path + `:${position.row + 1}` ;
      } else {
        secondaryLine.textContent = path;
      }
  ...
  async cancel() {
    if (this.theme == 'dock') { return; } // never cancel in Dock theme
  ...
  openTag(tag) {
    ...
    if (tag.file) {
      atom.workspace.open(path.join(tag.directory, tag.file))
      .then(() => this.moveToPosition(position));
    } else {
      if (this.theme == 'dock') { // need to activate the editor in Dock theme
        const editor = this.item.getEditor();
        ...
        if (editor.alive) {
          ...
          atom.workspace.open(editor.getURI())
          .then((editor) => {
    ...
    if (previous) {
      if (this.stack.length == 0 || !this.stack[this.stack.length - 1].position.isEqual(position)) {
  ...
  moveToPosition(position, beginningOfLine) {
    ...
    if (this.theme == 'dock') {
      if (!editor) { editor = this.item.getEditor(); }
    }
  ...
  async attach(query) {
    ...
    if (this.theme == 'dock') {
      await atom.workspace.open(this.item);
    } else {
      this.panel.show();
    }
