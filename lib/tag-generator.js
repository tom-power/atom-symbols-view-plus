...
import CtagsPath from './plus/ctags-path';

export default class TagGenerator {
  constructor(path1, scopeName) {
    this.path = path1;
    this.scopeName = scopeName;
    this.ctagsPath = new CtagsPath();
    this.command = this.ctagsPath.getCommand();
    this.args = this.ctagsPath.getArgs();
    this.options = {};
  ...
  parseErrorLines(lines) {
    lines = lines.split(/\n|\r\n/)
      // Ignore warnings
      .filter((line) => !line.match(/.*Warning: /))
      // Ignore empty lines
      .filter((line) => line != '');
  ...
  generateProjectSymbols() {
    ...
    if (extraCommandArgumentsWhenGeneratingProjectSymbols) {
      _args.push(...extraCommandArgumentsWhenGeneratingProjectSymbols.split(' '));
    }

    _args.push('-n', '-o', '.tags', '-R', './');
